...
/// La transformation de rotation du dé (celle qui porte la perspective),
/// distincte de celle du rebond qui l'enveloppe.
final _spin = find.byWidgetPredicate((w) => w is Transform && w.transform.entry(3, 2) != 0).first;
...
void main() {
  testWidgets('un rollToken déclenche l\'animation de lancer', (tester) async {
    ...
    final rotate = tester.widget<Transform>(_spin);
  ...
  testWidgets('le même rollToken ne rejoue pas l\'animation lors d\'un simple changement de state', (tester) async {
    ...
    await tester.pumpAndSettle();
    final restRotation = tester.widget<Transform>(_spin).transform.getRotation().entry(0, 1);
  ...
  testWidgets('sans rollToken, le dé reste directement sur son inclinaison de repos (pas d\'animation)', (tester) async {
    ...
    final atFirstFrame = tester.widget<Transform>(_spin).transform.getRotation().entry(0, 1);

    await tester.pump(const Duration(milliseconds: 100));
    final afterDelay = tester.widget<Transform>(_spin).transform.getRotation().entry(0, 1);
