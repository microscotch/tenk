...
A second workflow, `.github/workflows/docs.yaml`, publishes the GitHub Pages site
(https://microscotch.github.io/tenk/) on every push to `main`: `docs/` rendered by Jekyll exactly as
the former branch-based publication did — `privacy-policy.html`, which the store listings link to,
keeps its URL — plus the Dart API docs generated by dartdoc under `/api/`. It pins dartdoc 9.0.9
(`dart pub global activate`): the SDK's bundled 9.0.6 crashes on Flutter's `@docImport` comments.
Generating it on the Pi takes ~8 minutes and ~30 MB, which is why it's built in CI, not committed.

A third workflow, `.github/workflows/server-image.yaml`, builds the online-game server image
(`server/Dockerfile`, amd64 **and** arm64, no emulation: the build stage cross-compiles) and, on `main`,
publishes it to GitHub Container Registry as `ghcr.io/microscotch/tenk-server` (`:latest` and
`:sha-<commit>`). It runs only when `server/**`, `lib/game/**` or the workflow itself change; on a pull
request it builds and smoke-tests the image (`/healthz`) without publishing. Its Docker actions are pinned
...
The Dart API docs need no manual step: `.github/workflows/docs.yaml` regenerates them with dartdoc
and publishes them under `/api/` on every push to `main`. Doc comments are their source, so keep
them accurate as part of the same batch.
